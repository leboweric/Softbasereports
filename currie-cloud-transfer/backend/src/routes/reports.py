...
@reports_bp.route('/currie-financial-model', methods=['GET'])
@jwt_required()
def get_currie_financial_model_live():
    """
    Generate Currie Financial Model report with LIVE data from ERP.
    This pulls directly from the dealer's Softbase database.
    Replicates the exact report format from the original Softbase Reports application.
    """
    user = get_current_user()
    dealer_id = request.args.get('dealer_id', type=int)
    ...
    if not connection:
        return jsonify({'error': 'No active ERP connection found for this dealer'}), 404
    ...
    try:
        ...
        adapter = AdapterFactory.get_adapter(
            connection.erp_type,
            {
                ...
                'username': connection.username,
                'password': connection.password_encrypted,  # TODO: Decrypt
                'schema': 'ben002'  # Softbase schema
            }
        )

        # Get full Currie Financial Model data
        start_date = date.fromisoformat(start_date_str)
        end_date = date.fromisoformat(end_date_str)

        report_data = adapter.get_currie_financial_model(
            start_date=start_date,
            end_date=end_date,
            dealer_name=dealer.name,
            num_locations=1  # TODO: Make configurable per dealer
        )
...
@reports_bp.route('/currie-metrics', methods=['GET'])
@jwt_required()
def get_currie_metrics_live():
    """
    Get operational metrics for Currie Financial Model with LIVE data.
    Includes AR Aging, Service Calls, Technician Count, Labor Metrics, Absorption Rate.
    """
    user = get_current_user()
    dealer_id = request.args.get('dealer_id', type=int)
    ...
    try:
        ...
        metrics = adapter.get_currie_metrics(start_date, end_date)
