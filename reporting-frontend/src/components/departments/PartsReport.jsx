...
const PartsReport = ({ user, onNavigate }) => {
  ...
  const fetchAwaitingInvoiceData = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(apiUrl('/api/reports/departments/parts/awaiting-invoice'), {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      })
      ...
      if (response.ok) {
        const data = await response.json()
        setAwaitingInvoiceData(data)
      }
    } catch (error) {
      console.error('Error fetching parts awaiting invoice data:', error)
    }
  }
  ...
  const fetchAwaitingInvoiceDetails = async () => {
    try {
      ...
      const token = localStorage.getItem('token')
      const response = await fetch(apiUrl('/api/reports/departments/parts/awaiting-invoice-details'), {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      })
      ...
      console.error('Error fetching parts awaiting invoice details:', error)
    } finally {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['WO#', 'Type', 'Customer', 'Unit', 'Make/Model', 'Technician', 'Completed', 'Days Waiting', 'Parts', 'Misc', 'Total']
    ...
    link.setAttribute('download', `parts_awaiting_invoice_${today}.csv`)
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Parts Work Orders Awaiting Invoice */}
      {awaitingInvoiceData && awaitingInvoiceData.count > 0 && (
        <Card 
          className={`border-2 cursor-pointer transition-all hover:shadow-lg ${awaitingInvoiceData.over_three_days > 0 ? 'border-orange-400 bg-orange-50 hover:border-orange-500' : 'border-yellow-400 bg-yellow-50 hover:border-yellow-500'}`}
          onClick={handleCardClick}
        >
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <CardTitle className="text-lg">Parts Work Orders Awaiting Invoice</CardTitle>
                {awaitingInvoiceData.over_three_days > 0 && (
                  <AlertTriangle className="h-5 w-5 text-orange-600" />
                )}
              </div>
              <Badge variant={awaitingInvoiceData.over_three_days > 0 ? "destructive" : "warning"}>
          ...
          <CardContent className="space-y-2">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-semibold text-lg">{formatCurrency(awaitingInvoiceData.total_value)}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Avg Days Waiting</p>
                <p className="font-semibold text-lg flex items-center gap-1">
                  <Clock className="h-4 w-4" />
                  {awaitingInvoiceData.avg_days_waiting.toFixed(1)} days
                </p>
              </div>
            </div>
            {awaitingInvoiceData.over_three_days > 0 && (
              <div className="pt-2 border-t">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-orange-700">
                    <strong>{awaitingInvoiceData.over_three_days}</strong> orders waiting >3 days
                    {awaitingInvoiceData.over_five_days > 0 && (
                      <span> ({awaitingInvoiceData.over_five_days} over 5 days{awaitingInvoiceData.over_seven_days > 0 && `, ${awaitingInvoiceData.over_seven_days} over 7 days`})</span>
                    )}
                  </span>
                </div>
              </div>
      ...
      {/* Parts Awaiting Invoice Details Modal */}
      <Dialog open={showAwaitingInvoiceModal} onOpenChange={setShowAwaitingInvoiceModal}>
        <DialogContent className="max-w-[90vw] max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle className="text-xl">Parts Work Orders Awaiting Invoice</DialogTitle>
            <DialogDescription>
              Detailed list of all completed parts work orders that have not been invoiced
            </DialogDescription>
          </DialogHeader>
