...
def register_department_routes(reports_bp):
    ...
    @reports_bp.route('/woquote-structure', methods=['GET'])
    def get_woquote_structure():
        """Debug endpoint to examine WOQuote table structure - no auth required"""
        try:
            db = get_db()
            ...
            # Get count of quotes by month
            monthly_count_query = """
            SELECT 
                YEAR(CreationTime) as year,
                MONTH(CreationTime) as month,
                COUNT(*) as quote_count,
                COUNT(DISTINCT WONo) as unique_wo_count,
            ...
            """
            monthly_counts = db.execute_query(monthly_count_query)
            ...
            SELECT TOP 20
                WONo,
                COUNT(*) as quote_count,
                MIN(CreationTime) as first_quote,
                MAX(CreationTime) as last_quote,
                SUM(Amount) as total_amount,
            ...
            ORDER BY COUNT(*) DESC
            """
            duplicates = db.execute_query(duplicate_check_query)
            ...
            return jsonify({
                ...
                'message': 'WOQuote table structure and sample data'
            })
