...
@vital_finance_bp.route('/api/vital/finance/pivot/current-value-renewals', methods=['GET'])
@jwt_required()
def get_current_value_renewals_pivot():
    """
    Get Current Value Renewals Pivot data.
    ...
    try:
        ...
        engine = BillingEngine(conn)
        result = engine.get_current_value_renewals_pivot(
            org_id=org_id,
            year=year,
            revenue_timing=revenue_timing
        )
