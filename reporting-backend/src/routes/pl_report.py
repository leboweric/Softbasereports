...
@pl_report_bp.route('/api/reports/pl/discover-accounts', methods=['GET'])
@jwt_required()
def discover_accounts():
    """
    Temporary endpoint to discover all GL accounts for a tenant.
    Returns accounts grouped by prefix, excluding known revenue/COGS accounts.
    """
    try:
        schema = get_tenant_schema()
        tenant_gl_accounts = get_gl_accounts(schema)
        
        # Collect all known revenue and COGS accounts
        known_accounts = set()
        for dept in tenant_gl_accounts.values():
            known_accounts.update(dept.get('revenue', []))
            known_accounts.update(dept.get('cogs', []))
            known_accounts.update(dept.get('counter_revenue', []))
            known_accounts.update(dept.get('counter_cogs', []))
            known_accounts.update(dept.get('repair_order_revenue', []))
            known_accounts.update(dept.get('repair_order_cogs', []))
            known_accounts.update(dept.get('field_revenue', []))
            known_accounts.update(dept.get('field_cogs', []))
            known_accounts.update(dept.get('shop_revenue', []))
            known_accounts.update(dept.get('shop_cogs', []))
        
        # Also add other income accounts
        other_income = get_other_income_accounts(schema)
        known_accounts.update(other_income)
        
        # Query ALL distinct accounts with activity in 2025
        query = f"""
        SELECT DISTINCT AccountNo, MTD, YTD
        FROM {schema}.GL
        WHERE Year = 2025
          AND (MTD != 0 OR YTD != 0)
        ORDER BY AccountNo
        ...
        results = get_sql_service().execute_query(query, [])
        ...
        for row in results:
            acct = str(row['AccountNo']).strip()
            mtd = float(row.get('MTD', 0) or 0)
            ytd = float(row.get('YTD', 0) or 0)
            all_accounts.append({'account': acct, 'mtd': mtd, 'ytd': ytd})
            if acct not in known_accounts:
                unknown_accounts.append({'account': acct, 'mtd': mtd, 'ytd': ytd})
        ...
        logger.error(f"Error discovering accounts: {str(e)}")
